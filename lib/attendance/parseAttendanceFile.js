...
const SALCEDO_ATTLOG_OFFSET_MINUTES = -20;

function shouldApplySalcedoAttlogOffset(branchCode = "", branchName = "") {
  return [branchCode, branchName].some((value) =>
    normalizeText(value).includes("salcedo")
  );
}

function applyMinutesOffset(date, minutes) {
...
function parseAttlogDat({ buffer, fileName, branchCode, branchName }) {
  ...
  const groups = new Map();
  const logs = [];
  const shouldShiftSalcedoPunches = shouldApplySalcedoAttlogOffset(branchCode, branchName);
  let month = null;
  let year = null;
  ...
  for (const line of lines) {
    ...
    const parsedPunchedAt =
      parts[1] && parts[2] ? parseLocalEcuadorDateTime(`${parts[1]} ${parts[2]}`) : null;
    const punchedAt = parsedPunchedAt && shouldShiftSalcedoPunches
      ? applyMinutesOffset(parsedPunchedAt, SALCEDO_ATTLOG_OFFSET_MINUTES)
      : parsedPunchedAt;
  ...
  if (shouldShiftSalcedoPunches && employees.length) {
    logs.push("Picadas de Salcedo ajustadas -20 minutos por desfase del biometrico.");
  }
