...
export function attendancePayrollPolicy(employee = {}, laborRules = {}) {
  ...
  const isPlannedAttendanceExemptArea = PLANNED_ATTENDANCE_EXEMPT_AREA_CODES.has(String(employee?.areaCode || "").trim().toUpperCase());

  if (!rule) {
    if (isPlannedAttendanceExemptArea) {
      return {
  ...
  return {
    scheduleAffectsSalary: isPlannedAttendanceExemptArea ? false : rule.scheduleAffectsSalary ?? false,
    appliesSupplementaryHours: isPlannedAttendanceExemptArea ? false : rule.appliesSupplementaryHours ?? false,
    appliesExtraordinaryHours: isPlannedAttendanceExemptArea ? false : rule.appliesExtraordinaryHours ?? false,
