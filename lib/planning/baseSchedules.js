...
export function normalizeTemplateRow(row) {
  ...
  return {
    ...normalizedRow,
    authorizedExtraMinutes: typeConfig.isWorkingDay ? calculateAutoSupplementaryMinutes(normalizedRow) : 0,
  };
}
  ...
  return {
    ...
    weeklyRows: (template.weeklyRows || []).map((row) => {
      ...
      return {
        ...
        endTime: row.endTime || "",
        authorizedExtraMinutes: calculateAutoSupplementaryMinutes(row),
