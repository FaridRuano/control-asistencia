...
function resolveAuthorizedSupplementaryMinutes(row, normalizedRow = row) {
  if (row && Object.hasOwn(row, "authorizedExtraMinutes")) {
    return normalizeNumber(row.authorizedExtraMinutes, 0, { min: 0, max: 720 });
  }

  return calculateAutoSupplementaryMinutes(normalizedRow);
}
...
export function normalizeTemplateRow(row) {
  ...
  return {
    ...normalizedRow,
    authorizedExtraMinutes: typeConfig.isWorkingDay ? resolveAuthorizedSupplementaryMinutes(row, normalizedRow) : 0,
  };
}
  ...
  return {
    ...
    weeklyRows: (template.weeklyRows || []).map((row) => {
      ...
      return {
        ...
        endTime: row.endTime || "",
        authorizedExtraMinutes: resolveAuthorizedSupplementaryMinutes(row),
