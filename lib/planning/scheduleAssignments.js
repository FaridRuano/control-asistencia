...
function cloneTemplateRow(row) {
  const hasAuthorizedExtraMinutes = row && Object.hasOwn(row, "authorizedExtraMinutes");
  const authorizedExtraMinutes = row?.dayType === "workday"
    ? Math.max(
      0,
      hasAuthorizedExtraMinutes
        ? Number(row.authorizedExtraMinutes) || 0
        : calculateNetMinutes(row) - 8 * 60,
    )
    : 0;
