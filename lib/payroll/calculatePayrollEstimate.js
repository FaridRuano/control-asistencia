...
function resolveAbsenceHours(day) {
  ...
  const isSinglePunchDay = Boolean(day.hasSinglePunchCandidate);
...
function resolveStatus(day, extraordinaryHours, supplementaryHours, absenceHours, lateDiscountMinutes) {
  ...
  if (day.hasSinglePunchCandidate && day.savedIncompleteDayDecision === "valid_day") {
    return "Día validado manualmente";
  }

  if (day.hasSinglePunchCandidate && !day.savedIncompleteDayDecision) {
...
export default function calculatePayrollEstimate({
  ...
  const hourlyRate = hasSalaryConfigured ? salary / MONTHLY_STANDARD_HOURS : 0;
  ...
  const rows = visibleDays.map((day) => {
    ...
    const normalPaidHours =
      day.schedule?.dayType === "vacation" || day.schedule?.dayType === "holiday"
        ? REGULAR_DAILY_HOURS
        : day.schedule?.dayType === "workday" && day.hasSinglePunchCandidate
    ...
    return {
      ...
      normalPaidHours,
      basePayAmount,
      basePayAmountLabel: hasSalaryConfigured ? formatMoney(basePayAmount) : "Error",
      supplementaryHours,
      extraordinaryHours,
      absenceHours,
      lateDiscountMinutes,
      ...
      adjustmentAmount,
      adjustmentAmountLabel: hasSalaryConfigured ? formatMoney(adjustmentAmount) : "Error",
    };
  });
  ...
  return {
    ...
    summary: {
      hasSalaryConfigured,
      salary,
      salaryLabel: hasSalaryConfigured ? formatMoney(salary) : "Error",
      hourlyRate,
      hourlyRateLabel: hasSalaryConfigured ? formatMoney(hourlyRate) : "Error",
      dailyRate,
      dailyRateLabel: hasSalaryConfigured ? formatMoney(dailyRate) : "Error",
      basePaidHours: basePaidHoursTotal,
      basePaidHoursLabel: formatHours(basePaidHoursTotal),
      basePayTotal,
      basePayTotalLabel: hasSalaryConfigured ? formatMoney(basePayTotal) : "Error",
      ...
      absenceDiscount,
      absenceDiscountLabel: hasSalaryConfigured ? formatMoney(absenceDiscount) : "Error",
      lateDiscountAmount,
      lateDiscountAmountLabel: hasSalaryConfigured ? formatMoney(lateDiscountAmount) : "Error",
      supplementaryPay,
      supplementaryPayLabel: hasSalaryConfigured ? formatMoney(supplementaryPay) : "Error",
      extraordinaryPay,
      extraordinaryPayLabel: hasSalaryConfigured ? formatMoney(extraordinaryPay) : "Error",
      estimatedSalary,
      estimatedSalaryLabel: hasSalaryConfigured ? formatMoney(estimatedSalary) : "Error",
    },
    rows,
  };
}
